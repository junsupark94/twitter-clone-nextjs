...
const SideBar: React.FC<SideBarProps> = () => {
  return (
    <header className="relative mx-2 mt-2">
      <div>
        <nav className="flex flex-col gap-1">
          ...
          <ul className="flex flex-col 2xl:items-center">
            {SideBarItems.map((item) => (
              <li key={item.type} className="my-1 rounded-full hover:bg-color-hover p-2">
                <Link
                  href={`/${item.type}`}
                  className={`flex items-center gap-4 2xl:justify-center `}
                >
